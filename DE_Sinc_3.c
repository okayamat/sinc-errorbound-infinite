...
double f(double t)
{
  return pow(M_PI_4 * t, M_PI_4) * exp(- M_PI_4 * t);
}

double ftrans(double y)
{
  double x = M_PI*sinh(y);
  double t;
  if (x >= 0) {
    t = x + log1p(exp(- x));
  } else {
  ...
  return pow(M_PI_4 * t, M_PI_4) / pow(1 + exp(x), M_PI_4);
}
...
{
  int k;
  double h = log(2 * d * N / mu) / N;
  double x = asinh(M_1_PI * log(expm1(t)));
  double val = 0;
